...
class Gallery_banner extends Connection {
    private $id;
    private $title;
    private $gallery_image;
    private $table_name ="gallery_banner";
    private static $instance;

    public static function getInstance() {
        if (!isset(self::$instance) || is_null(self::$instance)) {
            try {
                self::$instance = new Gallery_banner();
            } catch (Exception $ex) {
    ...
    public function add() {
        $sql = "INSERT INTO " . $this->table_name . " (title, gallery_image) VALUES(:title, :gallery_image)";
    ...
    public function update() {
        $sql = "UPDATE " . $this->table_name . " SET title = :title, gallery_image = :gallery_image  WHERE id = :id";
        $statement = $this->getDbConnection()->prepare($sql);
